...
https://www.youtube.com/watch?v=ZHexy3JW2JA   yeetcode
19 subscribers
...
public class CreateMaximumNumber {
	...
	public int[] findLexMax(int[] nums, int k) {
		...
		/*
		 * we have to form a k digit number which is lexicographically greatest that
		 * gives some hint that we need to maintain a monotonic decreasing stack and
		 * rem=n-k means we will remove n-k smaller digits from stack lets say
		 * nums=[6,7,4,8,9] and k=3, we have to remove 5-3=2 digits from the stack,
		 * while we try to form monotonic decreasing satck o our max number will be
		 * [8,9]
		 */
		int rem = n - k;
		for (int i = 0; i < n; i++) {
		 ...
		 * elements from the top of the stack, as we are mainting a monotonic decreasing
		 * satck so elemets at the top would be smaller take this example arr-[6,4,2]
		 * k=2 ans would be [6,4]
		 */
		while (rem > 0) {
			st.pop();
			rem--;
	...
	private static boolean findMax(int[] arr1, int[] arr2, int p1, int p2) {
		...
		// when p1 is empty returns true, so that in merge function it picks up values
		// of arr2
		// when p1==arr1.length, then returns false,so that we keep pn picking arr2
		// elements
		return p1 != arr1.length; // tricky
	}

	// think of the case [6,0,4] and [6,7], can t do simple logic like merge two
	// sorted arrays
	// we need to decide which pointer to move forward based on the rest of the
	// array,
	...
	public int[] maxNumber(int[] nums1, int[] nums2, int k) {
		...
		for (int i = 0; i <= k; i++) { // skip invalid cases, imp step!
			int j = k - i;
			if (i <= len1 && (k - i) <= len2) {
